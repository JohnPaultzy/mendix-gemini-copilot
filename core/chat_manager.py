...
def delete_single_message(message_id):
    """Papason ang specific nga mensahe pinaagi sa message_id."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def branch_session_from_message(current_session_id, message_id, new_session_id):
    """Mokopya sa chat history gikan sa sinugdanan hangtod sa gipiling mensahe padulong sa bag-ong session."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # 1. Kuhaon ang session details
